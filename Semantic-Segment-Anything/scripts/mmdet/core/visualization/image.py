...
from mmdet.core.visualization.keywords import keywords
...
def clean_keywords(keywords):
    ...
    # 定義定冠詞
    articles = {'a', 'an', 'the', "araffed"}
    colors = {'red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet', 'white', 'black', 'gray', 'brown', 'pink', 'purple'}
    other_stop_words = {'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'araffed', 'blurry'}
    stop_words = set(stopwords.words('english')) | articles | colors | other_stop_words
    
    # 定義形容詞的詞性標籤
    adjective_tags = {'JJ', 'JJR', 'JJS'}  # 普通形容詞, 比較級, 最高級
    ...
    for keyword in keywords:
        ...
        # 去掉形容詞
        non_adjective_tokens = [word for word, tag in tagged_tokens if tag not in adjective_tags]
        
        # 轉為單數
        singular_tokens = [p.singular_noun(token) if p.singular_noun(token) else token for token in non_adjective_tokens]
        
        # 合併 tokens 為字串
        cleaned_keyword = ' '.join(singular_tokens)
...
# TODO: Modify this function to accept a dictionary of keywords
def map_class_names_to_categories(class_names: list, keywords: dict) -> str:
    """
    Maps the given class_name to a category in the keywords dictionary.

    Parameters:
        class_names (list<str>): The names of the classes to map.
        keywords (dict): A dictionary where keys are categories and values are lists of keywords.

    Returns:
        list<str>: The category names if found, otherwise 'other_objects'.
    """

    # Clean the keywords
    cleaned_keywords = clean_keywords(class_names)

    result = []

    for class_name in cleaned_keywords:
        category_found = "other_objects"
        for category, keyword_list in keywords.items():
            if class_name in keyword_list:
                category_found = category
                break
        result.append(category_found)
...
def draw_masks(ax, img, masks, class_names=None, color=None, with_edge=True, alpha=0.8):
    ...
    for i, (mask, class_name) in enumerate(zip(masks, class_names)):
        ...
        # # TODO: Filter out useless class
        # if 'car' not in class_name: continue
...
def imshow_det_bboxes(img,
    ...
    if segms is not None:
        ...
        # TODO: assign meaningful colors(e.g. one class one color), currently random by default

        color_list = np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [0, 255, 255], [255, 0, 255], [255, 255, 255], [0, 0, 0]], dtype=np.uint8)

        category_list = [category for category in keywords.keys()]
        
        categories = map_class_names_to_categories(class_names, keywords=keywords)
        ...
        for category in categories:
            if category not in category_list:
                ...
                colors.append(color_list[category_list.index(category)])

        # TODO: adding more labels ; note: adding class_names input
        draw_masks(ax, img, segms, class_names=categories, color=colors, with_edge=True)
